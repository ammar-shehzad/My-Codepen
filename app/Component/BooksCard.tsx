...
interface BookCardProp {
  myFile: Book;
//   setmyFile:Dispatch<SetStateAction<Book[]>>
}

const BookCard: React.FC<BookCardProp> = ({ myFile }) => {
  return (
    <>
      <style>{myFile.Css}</style>
      ...
      <div className="bg-gray-600 block max-w-sm border border-default rounded-base shadow-xs p-5">
        ...
        <div className="p-6 text-center">
          ...
          <a href="#">
            <h5 className="mt-3 mb-6 text-2xl font-semibold tracking-tight text-heading text-white">
              {myFile.BookName}
            </h5>
          </a>
          <a href="#">
            <p className="mt-3 mb-6 text-lg font-semibold tracking-tight text-heading text-white">
              {myFile.userName}
            </p>
          </a>
          <Link
            href={`viewcode/${myFile.id}`}
